...
await run('D.1 — parses a single lesson', () => {
  ...
  fs.mkdirSync(path.join(dir, 'harness'), { recursive: true }); fs.writeFileSync(path.join(dir, 'harness', 'progress.md'), [
    ...
    '## Lessons',
    '',
    '2026-06-18 | Hermes | Fixed token refresh bug',
  ].join('\n'), 'utf-8');
  const result = progress.readProgress(dir);
  assertEqual(result.lessons.length, 1);
  assertEqual(result.lessons[0].date, '2026-06-18');
  assertEqual(result.lessons[0].author, 'Hermes');
...
await run('D.2 — parses multiple lessons', () => {
  ...
  fs.mkdirSync(path.join(dir, 'harness'), { recursive: true }); fs.writeFileSync(path.join(dir, 'harness', 'progress.md'), [
    '## Lessons',
    '',
    '2026-06-18 | Hermes | First lesson',
...
await run('D.3 — lesson with trailing whitespace', () => {
  ...
  fs.mkdirSync(path.join(dir, 'harness'), { recursive: true }); fs.writeFileSync(path.join(dir, 'harness', 'progress.md'), [
    '## Lessons',
    '',
    '2026-06-18 | Hermes | Some lesson   ',  // trailing spaces
  ].join('\n'), 'utf-8');
  const result = progress.readProgress(dir);
  assertEqual(result.lessons.length, 1);
...
await run('D.4 — lessons with pipe characters in text', () => {
  ...
  fs.mkdirSync(path.join(dir, 'harness'), { recursive: true }); fs.writeFileSync(path.join(dir, 'harness', 'progress.md'), [
    '## Lessons',
    '',
    '2026-06-18 | Hermes | Use | as separator in config',
  ].join('\n'), 'utf-8');
  const result = progress.readProgress(dir);
  assertEqual(result.lessons.length, 1);
...
await run('D.6 — malformed lesson line is skipped', () => {
  ...
  fs.mkdirSync(path.join(dir, 'harness'), { recursive: true }); fs.writeFileSync(path.join(dir, 'harness', 'progress.md'), [
    ...
    '2026-06-18 | Hermes | Valid lesson',
    'Also not a lesson',
  ].join('\n'), 'utf-8');
  const result = progress.readProgress(dir);
  assertEqual(result.lessons.length, 1);
...
await run('E.4 — preserves Lessons section when replacing Session State', () => {
  ...
  progress.appendLesson(dir, 'Test lesson', 'Hermes');
...
await run('E.6 — inserts Session State before first ## header if missing', () => {
  ...
  fs.mkdirSync(path.join(dir, 'harness'), { recursive: true }); fs.writeFileSync(path.join(dir, 'harness', 'progress.md'), [
    ...
    '## Lessons',
    '',
    '2026-06-18 | Hermes | Existing lesson',
  ].join('\n'), 'utf-8');
...
await run('H.5 — Lesson date boundary: YYYY-MM-DD validation', () => {
  ...
  fs.mkdirSync(path.join(dir, 'harness'), { recursive: true }); fs.writeFileSync(path.join(dir, 'harness', 'progress.md'), [
    '## Lessons',
    '',
    '9999-99-99 | Hermes | Future lesson',
...
await run('I.1 — template fields match DEFAULT_SESSION_FIELDS', () => {
  ...
  for (const [key, val] of Object.entries(defaults)) {
    // Check that the template has the key:value pattern
    // Note: template may have {{maxRetries}} instead of '3'
    const expectedInTemplate = val === '0/3' ? `0/{{maxRetries}}` : `${key}: ${val}`;
    // We just check the key is present, actual rendered value depends on template vars
    assertMatch(tmpl, new RegExp(`${key}:`), `template missing key: ${key}`);
  }
});
