#!/usr/bin/env node
/**
 * T9 — Outer Ralph Loop Engine Test Battery
 *
 * Tests ralph-outer.mjs functions directly (unit tests) and
...
async function suiteG() {
  ...
  await run('G.1 — ralph-outer exports continuePipeline and runAutopilot', async () => {
  ...
  await run('G.2 — phase.mjs consumes ralph-outer exports correctly', async () => {
  ...
  await run('G.6 — ralph-inner getPhaseType covers all phases in order', async () => {
    const { getPhaseType } = ralphInner;
...
async function suiteI() {
  ...
  await run('I.2 — T10: Escalation on maxRetries exceeded IS implemented (T9 gap closed by T10)', async () => {
    ...
    const result = await ralphInner.runPhase(dir, 'define', { json: true });
    assertEqual(result.status, 'escalated', 'T10 implements escalation when retries exhausted');
    assertEqual(result.details.retryCount, 3, 'retryCount tracked');
    assertEqual(result.details.maxRetries, 3, 'maxRetries config active');
    assertOk(result.message.includes('Escalating'), 'message contains escalation notice');
  });
  ...
  await run('I.3 — T10: Git auto-tag on pipeline iteration IS implemented (T9 gap closed by T10)', async () => {
    ...
    assert(source.includes('autoTag'), 'T10 implements auto-tag in ralph-outer.mjs');
  });
