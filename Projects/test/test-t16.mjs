 ...
 *   - Cross-references: evaluator.md, AGENTS.md, ralph-inner.mjs
...
function testCrossReferences() {
  ...
  // F3: ralph-output.mjs (extracted from ralph-inner.mjs) should reference rubric in REVIEW phase
  const ralphOutput = readFileSync(resolve(PROJ_DIR, 'cli/lib/ralph-output.mjs'), 'utf-8');
  assertIncludes(ralphOutput, 'evaluator-rubric.md', 'F.3 ralph-output.mjs references evaluator-rubric.md');

  // F4: gates.mjs should have rubric check in REVIEW phase
  const gates = readFileSync(resolve(PROJ_DIR, 'cli/lib/gates.mjs'), 'utf-8');
  assertIncludes(gates, 'rubric-exists', 'F.4 gates.mjs has rubric-exists check');
...
function testReviewGateCheck() {
  ...
  const rubricCheck = validateOutput.checks?.find(c => c.name === 'rubric-exists');
  assertP(!!rubricCheck, 'H.1 validate --phase review includes rubric-exists check');
  assertP(rubricCheck.pass, 'H.2 rubric-exists passes when evaluator-rubric.md exists');
  ...
  const failCheck = failResult.checks?.find(c => c.name === 'rubric-exists');
  assertP(!!failCheck, 'H.3 rubric-exists check present even without file');
  assertP(!failCheck.pass, 'H.4 rubric-exists fails when evaluator-rubric.md is missing');
