...
export default async function selectToolCommand(args) {
  ...
  if (listOnly) {
    if (json) {
      ...
      process.stdout.write('  Tier 1 — Deep Integration (spawnable, session isolation):\n');
      for (const tool of TIER1_TOOLS) {
        ...
        process.stdout.write(`    ${icon} ${tool.label.padEnd(16)} ${tool.description}\n`);
        process.stdout.write(`        Capabilities: ${caps}\n`);
        process.stdout.write(`        Status: ${installed ? 'installed' : 'not installed'}\n\n`);
      }
      process.stdout.write('  Tier 2 — Instruction-Based (IDE extensions, no spawning):\n');
      for (const tool of TIER2_TOOLS) {
        process.stdout.write(`    ☐ ${tool.label.padEnd(16)} ${tool.description}\n`);
      }
      process.stdout.write('\n');
    }
    return;
  ...
  if (directSelection) {
    ...
    if (!found) {
      ...
      process.stderr.write(`Available: ${allTools.map(t => t.name).join(', ')}\n`);
      process.exit(1);
      return;
    }
    ...
    if (json) {
      ...
      if (isTier1) {
        process.stdout.write(`  Tier 1 — supports autonomous spawning (dev-harness run)\n`);
      } else {
        process.stdout.write(`  Tier 2 — instruction-based (manual workflow)\n`);
      }
    }
    return;
  }
  ...
  // ── Interactive wizard ──────────────────────────────────────
  if (json) {
    process.stderr.write('Error: Interactive wizard not available in JSON mode. Use: dev-harness select-tool <tool-name>\n');
    process.exit(1);
    return;
  }

  process.stdout.write('\n  🧰 Backend Tool Selection Wizard\n\n');
  process.stdout.write('  Tier 1 — Deep Integration (supports dev-harness run):\n\n');
  ...
  for (let i = 0; i < tier1WithStatus.length; i++) {
    ...
    process.stdout.write(`    [${i + 1}] ${icon} ${tool.label} — ${tool.description}\n`);
    process.stdout.write(`        Status: ${tool.installed ? 'installed ✅' : 'not installed ❌'}\n`);
  }

  process.stdout.write('\n  Tier 2 — Instruction-Based (IDE tools, manual workflow):\n');
  process.stdout.write('    (Use these if you prefer working inside an IDE)\n\n');
  for (let i = 0; i < TIER2_TOOLS.length; i++) {
    const tool = TIER2_TOOLS[i];
    process.stdout.write(`    [${tier1WithStatus.length + i + 1}] ☐ ${tool.label} — ${tool.description}\n`);
  }
  ...
  if (isTier1) {
    process.stdout.write(`  Tier 1 — you can use: dev-harness run --agent-tool ${selected.name}\n`);
    if (!selected.installed) {
      process.stdout.write(`  ⚠ Note: '${selected.command}' not found on PATH. Install it before running.\n`);
    }
  } else {
    process.stdout.write(`  Tier 2 — use manual workflow: dev-harness phase <name> → validate → repeat\n`);
  }
  process.stdout.write('\n');
}
