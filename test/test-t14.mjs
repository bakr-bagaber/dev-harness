...
await run('A.4 — propose JSON output contract', () => {
  const dir = createProject();
  // G5: --criteria is now required
  const r = cli('contract propose --scope "Test" --criteria "tests pass" --json', dir);
...
await run('G.6 — validate honors gates.enabled false (DEFINE phase)', () => {
  const dir = createProject();
  // G12: gates are ON by default now; explicitly disable for this test
  cli('config set gates.enabled false', dir);
...
await run('H.3 — re-propose preserves evaluator review section', () => {
  const dir = createProject();
  cli('contract propose --scope "Login" --criteria "tests pass"', dir);
  cli('contract review --needs-revision --notes "Need more detail"', dir);
  // Re-propose with new scope
  cli('contract propose --scope "Login with OAuth" --criteria "tests pass"', dir);
  const content = readContract(dir);
