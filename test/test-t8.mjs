/**
 * test-t8.mjs — T8 Task Ralph Loop Engine (intentionally no dedicated tests).
 *
 * WHY THIS FILE HAS NO TESTS:
 * T8 created cli/lib/ralph-tasks.mjs — the task loop (work → validate →
 ...
 * suite covers the task loop comprehensively:
 ...
 *
 * Additionally T9 (phase loop) and T12 (autopilot) exercise await runPhase()
 * through the phase-loop auto-advance path.
 *
 * A separate T8 file would duplicate T10's 71 assertions. T10 is the
 * canonical task-loop test.
 *
...
const innerLoopFiles = [
  'cli/lib/ralph-tasks.mjs',
];

for (const rel of innerLoopFiles) {
  if (!existsSync(resolve(PROJECT_ROOT, rel))) {
    console.error(`  ✗ T8 task loop file missing: ${rel}`);
    process.exit(1);
  }
  passed++;
}

console.log('=== T8 Task Ralph Loop (covered by T10) ===');
console.log(`\nResults: ${passed} pass, 0 fail`);
console.log('  (T8 has no dedicated tests; task loop is tested in T10.)');
process.exit(0);
