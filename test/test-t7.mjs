...
await run('A.2 — define has 3 checks (feature-branch, contract-agreed, contract-criteria)', async () => {
  const dir = createProject();
  initGitRepo(dir);
  ...
  assertEqual(result.checks[2].name, 'contract-criteria');
});
...
await run('A.4 — build has 7 checks (git-clean, lint, tests, contract-agreed, contract-criteria, coverage, anti-placeholder)', async () => {
  const dir = createProject();
  initGitRepo(dir);
  const result = await gates.runChecks(dir, 'build');
  assertEqual(result.checks.length, 7);
  const names = result.checks.map(c => c.name);
  ...
  assert(names.includes('contract-criteria'), 'should include contract-criteria (G8)');
  assert(names.includes('coverage'), 'should include coverage');
  assert(names.includes('anti-placeholder'), 'should include anti-placeholder (G24b)');
});
...
await run('A.7 — review has 5 checks (branch-up-to-date, rubric-content, readme, architecture, decisions)', async () => {
  const dir = createProject();
  initGitRepo(dir);
  ...
  assert(names.includes('rubric-content'), 'should include rubric-content (G9: was rubric-exists)');
  assert(names.includes('readme-exists'));
  assert(names.includes('architecture-doc'));
  assert(names.includes('decisions-logged'));
});

await run('A.8 — ship has 9 checks (git-clean, tagged, changelog + deliverable gates + anti-placeholder)', async () => {
  const dir = createProject();
  initGitRepo(dir);
  const result = await gates.runChecks(dir, 'ship');
  assertEqual(result.checks.length, 9);
  const names = result.checks.map(c => c.name);
  assert(names.includes('git-clean'));
...
await run('B.3 — gates enabled by default when config missing (G12)', async () => {
  const dir = fs.mkdtempSync(path.join(TEST_TMP, 'no-config-'));
  // G12: gates ON by default — no config means defaults apply, which has enabled:true
  assertEqual(gates.areGatesEnabled(dir), false, 'no config file = areGatesEnabled returns false (loadConfig fails)');
});

await run('B.4 — gates enabled by default when config has no gates key (G12)', async () => {
  const dir = createProject();
  delete dir.gates; // hmm, can't do this
  // Use a config without gates key — deepMerge with defaults fills enabled:true
  ...
  assertEqual(gates.areGatesEnabled(dir2), true, 'no gates key = defaults apply = enabled (G12)');
});

await run('B.5 — gates enabled by default when enabled is undefined (G12)', async () => {
  const dir = createProject({ gates: { checks: ['all'] } });
  // deepMerge fills in enabled:true from defaults when not explicitly set
  assertEqual(gates.areGatesEnabled(dir), true, 'undefined = defaults apply = enabled (G12)');
});
...
await run('C.7 — build phase: lint/tests pass for generic stack', async () => {
  const dir = createProject();
  initGitRepo(dir);
  // Contract check requires sprint-contract.md with 'Agreed' status + criteria (G8)
