...
await run('A.1 — returns object with required fields', async () => {
  ...
  assert(cfg.gates.enabled === true, 'gates should be enabled by default (G12: enforcement by default)');
});

await run('A.2 — opt-in architecture: gates disabled, git auto ops disabled', async () => {
  const cfg = state.getDefaultConfig();
  assert(cfg.gates.enabled === true, 'gates.enabled not true (G12 default-on)');
...
await run('D.2 — extra fields preserved across deep merge', async () => {
  ...
  assert(result.config.gates.enabled === true, 'default gates missing (G12: on by default)');
});
...
await run('G.3 — get nested key (dot-notation)', async () => {
  ...
  assert(result.ok, `get failed: ${result.error}`);
  assert(result.value === true, `expected true (G12: gates on by default), got ${result.value}`);
});
...
await run('I.1 — config root fields preserved after deep set', async () => {
  ...
  assert(verify.value.gates.enabled === true, 'gates should be preserved (G12: on by default)');
