 ...
 *
 * Tests phase.mjs, ralph-inner.mjs, and ralph-outer.mjs integration:
...
async function loadModules() {
  if (!ralphOuter) {
    ralphOuter = await import(path.join(PROJECT_ROOT, 'cli/lib/ralph-outer.mjs'));
  }
  if (!ralphInner) {
    ralphInner = await import(path.join(PROJECT_ROOT, 'cli/lib/ralph-inner.mjs'));
  }
  if (!state) {
...
async function suiteD() {
  ...
  await run('D.3 — runPhase returns escalated when retryCount >= maxRetries', async () => {
    const dir = createProjectAtPhase('build', { maxRetries: 3, retryCount: 3 });
    writeFeatureList(dir, [
      { id: 'f1', name: 'F1', description: '', passes: false, tasks: [{ id: 't1', description: 'T1', status: 'pending' }] },
    ]);
    const result = await ralphInner.runPhase(dir, 'build', { json: true });
    assertEqual(result.status, 'escalated', 'should escalate when retries exhausted');
    assertEqual(result.ok, false, 'ok should be false on escalation');
    assertEqual(result.details.retryCount, 3);
    assertEqual(result.details.maxRetries, 3);
  });

  await run('D.4 — runPhase returns escalated when same-phase re-run exhausts retries (full flow)', async () => {
    const dir = createProjectAtPhase('build', { maxRetries: 2 });
    writeFeatureList(dir, [
      { id: 'f1', name: 'F1', description: '', passes: false, tasks: [{ id: 't1', description: 'T1', status: 'pending' }] },
    ]);
    // Simulate phase.mjs: transitionPhase then runPhase
    await state.transitionPhase(dir, 'build'); // retry: 1
    let r = await ralphInner.runPhase(dir, 'build', { json: true });
    assertEqual(r.status, 'instruction', 'first re-run should be instruction');

    await state.transitionPhase(dir, 'build'); // retry: 2
    r = await ralphInner.runPhase(dir, 'build', { json: true });
    assertEqual(r.status, 'escalated', 'second re-run should escalate (maxRetries=2)');
  });
...
async function suiteE() {
  ...
  // Test getPhaseType for all phases
  await run('E.2 — getPhaseType returns correct types for all phases', async () => {
    assertEqual(ralphInner.getPhaseType('init'), 'deliverable-retry');
    assertEqual(ralphInner.getPhaseType('define'), 'deliverable-retry');
    assertEqual(ralphInner.getPhaseType('plan'), 'deliverable-retry');
    assertEqual(ralphInner.getPhaseType('build'), 'feature-iterate');
    assertEqual(ralphInner.getPhaseType('verify'), 'feature-iterate');
    assertEqual(ralphInner.getPhaseType('simplify'), 'feature-iterate');
    assertEqual(ralphInner.getPhaseType('review'), 'deliverable-retry');
    assertEqual(ralphInner.getPhaseType('ship'), 'deliverable-retry');
    assertEqual(ralphInner.getPhaseType('unknown'), null);
  });
}
...
async function suiteG() {
  ...
  await run('G.3 — continuePipeline autopilot escalates on inner loop failure', async () => {
    const dir = createProjectAtPhase('build', { mode: 'autopilot', maxRetries: 1, retryCount: 1 });
    writeFeatureList(dir, [
      { id: 'f1', name: 'F1', description: '', passes: false, tasks: [{ id: 't1', description: 'T1', status: 'pending' }] },
    ]);
    const result = await continuePipeline(dir, 'build', { json: true, verbose: false });
    // continuePipeline transitions to next phase (verify) before running inner loop
...
async function suiteK() {
  ...
  await run('K.3 — phase.mjs uses correct imports from ralph-inner.mjs', async () => {
    const source = fs.readFileSync(path.join(PROJECT_ROOT, 'cli/commands/phase.mjs'), 'utf-8');
    assertMatch(source, /runPhase/);
    assertMatch(source, /getPhaseType/);
  });

  await run('K.4 — phase.mjs uses correct imports from ralph-outer.mjs', async () => {
    const source = fs.readFileSync(path.join(PROJECT_ROOT, 'cli/commands/phase.mjs'), 'utf-8');
    assertMatch(source, /continuePipeline/);
  });

  await run('K.5 — ralph-outer.mjs uses correct imports from ralph-inner.mjs', async () => {
    const source = fs.readFileSync(path.join(PROJECT_ROOT, 'cli/lib/ralph-outer.mjs'), 'utf-8');
    assertMatch(source, /runPhase/);
  });
...
async function suiteL() {
  ...
  await run('L.5 — getNextFeature returns null for empty list', async () => {
    assertEqual(ralphInner.getNextFeature([]), null);
  });
  ...
  await run('L.6 — getNextFeature returns first with passes=false', async () => {
    ...
    const result = ralphInner.getNextFeature(features);
  ...
  await run('L.7 — getNextTask returns first pending task', async () => {
    ...
    const result = ralphInner.getNextTask(feature);
  ...
  await run('L.8 — getNextTask returns null when no pending tasks', async () => {
    ...
    assertEqual(ralphInner.getNextTask(feature), null);
  });

  await run('L.9 — getNextTask returns null when no tasks', async () => {
    assertEqual(ralphInner.getNextTask({}), null);
  });

  await run('L.10 — runPhase with maxRetries=0 escalates immediately on same-phase re-run', async () => {
    const dir = createProjectAtPhase('build', { maxRetries: 0 });
    writeFeatureList(dir, [
      { id: 'f1', name: 'F1', description: '', passes: false, tasks: [{ id: 't1', description: 'T1', status: 'pending' }] },
    ]);
    // First transition + run should escalate with maxRetries=0 since retryCount starts at 0
    await state.transitionPhase(dir, 'build');
    const result = await ralphInner.runPhase(dir, 'build', { json: true });
    assertEqual(result.status, 'escalated');
  });
}
...
async function suiteN() {
  ...
  await run('N.1 — T10: Escalation IS implemented (T9 gap fixed)', async () => {
    const innerSrc = fs.readFileSync(path.join(PROJECT_ROOT, 'cli/lib/ralph-inner.mjs'), 'utf-8');
    assert(innerSrc.includes('escalate'), 'ralph-inner should now contain escalation');
    assert(innerSrc.includes('escalated'), 'ralph-inner should contain escalated status');
  });

  await run('N.2 — T10: Auto-tag IS implemented (T9 gap fixed)', async () => {
    const outerSrc = fs.readFileSync(path.join(PROJECT_ROOT, 'cli/lib/ralph-outer.mjs'), 'utf-8');
    assert(outerSrc.includes('autoTag'), 'ralph-outer should now contain auto-tag');
  });
  ...
  await run('N.4 — T10: Retry tracking goes beyond iteration=1 (T9 gap fixed)', async () => {
    ...
    await state.transitionPhase(dir, 'define');
    const result = await ralphInner.runPhase(dir, 'define', { json: true });
    assertEqual(result.status, 'escalated', 'retry exhaustion should escalate');
