/**
 * test-t19.mjs — T19 Skill Skill Wrapper.
 *
 * Tests the Skill adapter at adapters/skill/ (moved from skill/dev-harness/):
...
const SKILL_DIR = resolve(PROJECT_ROOT, 'adapters/skill');
...
// C.4 No duplicate template files (symlink means 0 files in skill templates)
...
// Approved tools: skill, claude-code, codex, cursor, opencode, antigravity, openclaw, generic
const expectedAdapters = [
  'skill', 'claude-code', 'cursor', 'codex', 'generic',
...
// D.7 tool-registry exports approved tools (8: generic + 7 approved)
const { KNOWN_TOOLS, TOOL_REGISTRY } = await import(`${PROJECT_ROOT}/cli/lib/tool-registry.mjs`);
ok('D.7 registry has 8 tools (generic + 7 approved)', KNOWN_TOOLS.length === 8);
...
ok('D.10 registry has skill with special=true', TOOL_REGISTRY['skill']?.special === true);
ok('D.11 registry has openclaw (AGENTS.md native, file=null)', TOOL_REGISTRY['openclaw']?.file === null);
